...
from . import segmentation_models_pytorch_4TorchLessThan120 as smp
...
def get_seg(src_path, out_path, img_name=None):
    ...
    # cascade参数
    weight_c1 = r'seg_net/models/weigh_and_id/TNSCUI/fold1_stage1_trained_on_size_256.pkl'
    weight_c2 = r'seg_net/models/weigh_and_id/TNSCUI/fold1_stage2_trained_on_size_512.pkl'
    ...
    with torch.no_grad():
        ...
        model_cascade1.load_state_dict(torch.load(weight_c1, map_location=torch.device('cpu')))
        if c1_tta:
        ...
        model_cascade2.load_state_dict(torch.load(weight_c2, map_location=torch.device('cpu')))
        if c2_tta:
...
if __name__ == '__main__':
    ...
    save_path = f'../output/{img_name}.png'
    print(save_path)
    print(img_path)
    get_seg(img_path, save_path)
